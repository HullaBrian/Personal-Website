...
share-img: /assets/img/weaponizing-wdac-killing-the-dreams-of-edr/my-child-will.jpg
...
While having the ability to execute this attack from a Linux machine is simple, it is not very realistic. Realistically, an adversary will be operating in the context of an already compromised host on the network and will be looking to expand their access into the internal network through deployed post-exploitation capability. This is where a purpose-built tool comes in. [**Krueger**](https://github.com/logangoins/Krueger) is a custom tool written in C# by [Logan Goins](https://x.com/_logangoins) specifically meant to be run in memory as part of post-exploitation lateral movement activities. The example below uses `inlineExecute-Assembly` (created by [@anthemtotheego](https://x.com/anthemtotheego)) to execute the .NET assembly in memory.
...
Unfortunately, detecting a policy being placed into these sensitive locations is not the best way of detecting this attack. While a policy in these locations is indeed a characteristic of the attack, a policy's presence in these locations is not inherently malicious. Therefore, measures should be taken to proactively scan WDAC policies so as to determine their integrity.

As mentioned earlier, unless an EDR vendor maintains a WHQL signature on their drivers, it is not what is specifically mentioned in the WDAC policy that makes it malicious. Rather, it is what is not mentioned. WDAC policies block execution by default - meaning that non-WHQL drivers will be blocked unless specifically allowed. Therefore, ensuring that detections are properly tailored to an organization's environment is crucial to being able to effectively detect this attack technique. With that being said, if WHQL drivers are employed by an EDR vendor, malicious WDAC policies must specifically reference either the publisher used in the EDR's signing certificate or file attributes specific to the vendor for successful attack execution.

To showcase an in depth example, below are the strings found within a compiled policy that will block EDR vendors that do not maintain WHQL signatures. While there is not much potential for detecting malicious policies based off of what is present in this scenario, as previously mentioned there is an opportunity to detect potentially malicious activity based off of what is not present - such as references to the EDR vendor publisher or file attributes of the binaries, drivers, and libraries necessary for the EDR's operation.

![alt text](/assets/img/weaponizing-wdac-killing-the-dreams-of-edr/strings-on-compiled-policy.png)

On the other hand, if the EDR vendor's drivers are WHQL signed, a malicious WDAC policy must specifically block a publisher or file attribute. This creates a better opportunity for detection now that specific strings will be included in the compiled policy. Note by extracting strings from the compiled binary you can only detect the files and attributes referenced - nothing regarding whether or not they are blocked. As seen in the below screenshot, creating a policy with the same attributes allowed and denied has no effect on the generated strings.

![alt text](/assets/img/weaponizing-wdac-killing-the-dreams-of-edr/strings-comparison.png)

While creating a detection just based off of strings could detect potentially malicious policies, there would be a significant number of false positives. Therefore, creating a detection for blocking specific files or file attributes would enhance the overall detection quality. After analyzing dumps of compiled WDAC policies, there were a few key observations made as to how compiled WDAC policies are structured.
